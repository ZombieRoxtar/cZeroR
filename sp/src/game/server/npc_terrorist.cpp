...
int g_fTerroristQuestion; // true if an idle enemy asked a question. Cleared when someone answers.

ConVar sk_terrorist_health("sk_terrorist_health", "0");
ConVar sk_terrorist_kick("sk_terrorist_kick", "0");
...
LINK_ENTITY_TO_CLASS(npc_terrorist, CNPC_Terrorist);

//---------------------------------------------------------
// Save/Restore
//---------------------------------------------------------
BEGIN_DATADESC(CNPC_Terrorist)
...
DEFINE_INPUTFUNC(FIELD_VOID, "HitBySmoke", InputHitBySmoke),
DEFINE_INPUTFUNC(FIELD_STRING, "ThrowGrenadeAtTarget", InputThrowGrenadeAtTarget),

DEFINE_OUTPUT(m_OnBlinded, "OnBlinded"),
...
// Create components
//-----------------------------------------------------------------------------
bool CNPC_Terrorist::CreateComponents()
{
...
//------------------------------------------------------------------------------
void CNPC_Terrorist::InputLookOff(inputdata_t &inputdata)
{
	m_spawnflags |= SF_TERRORIST_NO_LOOK;
}

//------------------------------------------------------------------------------
// Purpose: Enable looking.
//------------------------------------------------------------------------------
void CNPC_Terrorist::InputLookOn(inputdata_t &inputdata)
{
	m_spawnflags &= ~SF_TERRORIST_NO_LOOK;
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Terrorist::InputStartPatrolling(inputdata_t &inputdata)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Terrorist::InputStopPatrolling(inputdata_t &inputdata)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Terrorist::InputAssault(inputdata_t &inputdata)
{
	m_AssaultBehavior.SetParameters(AllocPooledString(inputdata.value.String()), CUE_DONT_WAIT, RALLY_POINT_SELECT_DEFAULT);
}

//-----------------------------------------------------------------------------
// We were "hit" by a flashbang or smoke grenade
//-----------------------------------------------------------------------------
void CNPC_Terrorist::InputHitBySmoke(inputdata_t &inputdata)
{
	SetCondition(COND_COMBINE_HIT_BY_SMOKE);
}

//-----------------------------------------------------------------------------
// Purpose: Force a grenade throw at the target
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_Terrorist::InputThrowGrenadeAtTarget(inputdata_t &inputdata)
{
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_Terrorist::Precache()
{
	/* Is this the place to set up the terrorists' races?
	if( !Q_stricmp( STRING( GetModelName()), "models/combine_super_soldier.mdl" ) )
	{
	m_fIsElite = true;
	}
	else
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Terrorist::Spawn(void)
{

	Precache();
	SetModel(STRING(GetModelName()));

	SetHealth(sk_terrorist_health.GetFloat());
	SetMaxHealth(sk_terrorist_health.GetFloat());
	SetKickDamage(sk_terrorist_kick.GetFloat());
	...
	m_bShouldPatrol = false;
	m_flNextPainSoundTime, m_flNextAlertSoundTime = 0;
	...
	m_bFirstEncounter = true; // this is true on spawn, because we haven't encountered an enemy yet.
	...
	m_flNextLostSoundTime, m_flAlertPatrolTime = 0;
...
//-----------------------------------------------------------------------------
bool CNPC_Terrorist::CreateBehaviors()
{
...
//-----------------------------------------------------------------------------
void CNPC_Terrorist::GatherConditions()
{
	...
	if (GetState() == NPC_STATE_COMBAT)
	{
		...
		{
			// T's that are standing around doing nothing poll for attack slots so
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Terrorist::PrescheduleThink()
{
...
//-----------------------------------------------------------------------------
float CNPC_Terrorist::MaxYawSpeed(void)
{
	switch (GetActivity())
	{
	case ACT_TURN_LEFT:
...
//-----------------------------------------------------------------------------
bool CNPC_Terrorist::ShouldMoveAndShoot()
{
...
// Output :
//-----------------------------------------------------------------------------
bool CNPC_Terrorist::OverrideMoveFacing(const AILocalMoveGoal_t &move, float flInterval)
{
...
//
//-----------------------------------------------------------------------------
Class_T	CNPC_Terrorist::Classify(void)
{
...
//-----------------------------------------------------------------------------
bool CNPC_Terrorist::IsCurTaskContinuousMove()
{
...
//-----------------------------------------------------------------------------
void CNPC_Terrorist::StartTaskChaseEnemyContinuously(const Task_t *pTask)
{
...
void CNPC_Terrorist::RunTaskChaseEnemyContinuously(const Task_t *pTask)
{
	if (!GetNavigator()->IsGoalActive())
	{
		SetIdealActivity(GetStoppedActivity());
...
// start task
//=========================================================
void CNPC_Terrorist::StartTask(const Task_t *pTask)
{
	...
	case TASK_WALK_PATH:
	case TASK_RUN_PATH:
		// No longer assume he is covered if he moves
		Forget(bits_MEMORY_INCOVER);
	...
	case TASK_COMBINE_DEFER_SQUAD_GRENADES:
	{
		if (m_pSquad)
		{
			...
			while (pSquadmate)
			{
				CNPC_Terrorist *pCombine = dynamic_cast<CNPC_Terrorist*>(pSquadmate);
...
// RunTask
//=========================================================
void CNPC_Terrorist::RunTask(const Task_t *pTask)
{
	switch (pTask->iTask)
	{
	case TASK_COMBINE_CHASE_ENEMY_CONTINUOUSLY:
...
// Output  :
//------------------------------------------------------------------------------
Vector CNPC_Terrorist::BodyTarget(const Vector &posSrc, bool bNoisy)
{
...
// Purpose:
//------------------------------------------------------------------------------
bool CNPC_Terrorist::FVisible(CBaseEntity *pEntity, int traceMask, CBaseEntity **ppBlocker)
{
	if (m_spawnflags & SF_TERRORIST_NO_LOOK)
	{
...
//-----------------------------------------------------------------------------
float CNPC_Terrorist::GetHitgroupDamageMultiplier(int iHitGroup, const CTakeDamageInfo &info)
{
	switch (iHitGroup)
	{
	case HITGROUP_HEAD:
...
//-----------------------------------------------------------------------------
void CNPC_Terrorist::Event_Killed(const CTakeDamageInfo &info)
{
...
//-----------------------------------------------------------------------------
bool CNPC_Terrorist::UpdateEnemyMemory(CBaseEntity *pEnemy, const Vector &position, CBaseEntity *pInformer)
{
	if (m_spawnflags & SF_TERRORIST_NO_LOOK)
	{
		return false;
...
//-----------------------------------------------------------------------------
void CNPC_Terrorist::BuildScheduleTestBits(void)
{
	...
	SetCustomInterruptCondition(COND_COMBINE_HIT_BY_SMOKE);
...
//-----------------------------------------------------------------------------
Activity CNPC_Terrorist::NPC_TranslateActivity(Activity eNewActivity)
{
	//Slaming this back to ACT_TERRORIST_BLINDED since we don't want ANYTHING to change our activity while we burn.
	if (HasCondition(COND_COMBINE_ON_FIRE))
		return BaseClass::NPC_TranslateActivity(ACT_TERRORIST_BLINDED);

	if (eNewActivity == ACT_RANGE_ATTACK2)
	{
		eNewActivity = ACT_TERRORIST_THROW_GRENADE;
	}
...
//-----------------------------------------------------------------------------
// Purpose: Overidden because they hear the DANGER sound
// Input  :
// Output :
//-----------------------------------------------------------------------------
int CNPC_Terrorist::GetSoundInterests(void)
{
...
//-----------------------------------------------------------------------------
bool CNPC_Terrorist::QueryHearSound(CSound *pSound)
{
...
//-----------------------------------------------------------------------------
void CNPC_Terrorist::OnListened()
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_Terrorist::AnnounceAssault(void)
{
...
void CNPC_Terrorist::AnnounceEnemyType(CBaseEntity *pEnemy)
{
	const char *pSentenceName = "COMBINE_MONST_CITIZENS";
...
void CNPC_Terrorist::AnnounceEnemyKill(CBaseEntity *pEnemy)
{
	if (!pEnemy)
		return;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_Terrorist::IsHeavyDamage(const CTakeDamageInfo &info)
{
	if (info.GetAmmoType() == GetAmmoDef()->Index("338Mag"))
		return true;
...
//-----------------------------------------------------------------------------
int CNPC_Terrorist::SelectCombatSchedule()
{
	// -----------
	// dead enemy
	// -----------
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_Terrorist::SelectSchedule(void)
{
	...
	{
		// If we're hit by a Smoke/FlashBang, thrash around
		if (HasCondition(COND_COMBINE_HIT_BY_SMOKE))
		{
			...
			m_OnBlinded.FireOutput(UTIL_GetLocalPlayer(), this); /* FIXME: Get the guy who threw the grenade. If a NPC can do it then this is a big bug! */
			ClearCondition(COND_COMBINE_HIT_BY_SMOKE);
			return SCHED_COMBINE_SMOKE_DISTRACTION;
		}
...
//-----------------------------------------------------------------------------
int CNPC_Terrorist::SelectFailSchedule(int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode)
{
...
//-----------------------------------------------------------------------------
int CNPC_Terrorist::SelectScheduleAttack()
{
	// Drop a grenade?
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_Terrorist::TranslateSchedule(int scheduleType)
{
	switch (scheduleType)
	{
...
//=========================================================
void CNPC_Terrorist::OnStartSchedule(int scheduleType)
{
}
...
//=========================================================
void CNPC_Terrorist::HandleAnimEvent(animevent_t *pEvent)
{
	Vector vecShootDir;
...
// Output :
//-----------------------------------------------------------------------------
Vector CNPC_Terrorist::Weapon_ShootPosition()
{
...
//
// Some sentences (take cover and charge) rely on actually
// being able to execute the intended action. It's really lame
// when one says 'COVER ME' and then doesn't move. The problem
// is that the sentences were played when the decision to TRY
// to move to cover was made. Now the sentence is played after 
// we know for sure that there is a valid path. The schedule
// may still fail but in most cases, well after we have 
// started moving.
//=========================================================
void CNPC_Terrorist::SpeakSentence(int sentenceType)
{
	switch (sentenceType)
	{
	case 0: // assault
...
// PainSound
//=========================================================
void CNPC_Terrorist::PainSound(void)
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_Terrorist::LostEnemySound(void)
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_Terrorist::FoundEnemySound(void)
{
...
void CNPC_Terrorist::AlertSound(void)
{
...
// NotifyDeadFriend
//=========================================================
void CNPC_Terrorist::NotifyDeadFriend(CBaseEntity* pFriend)
{
...
// DeathSound 
//=========================================================
void CNPC_Terrorist::DeathSound(void)
{
	// NOTE: The response system deals with this at the moment
	if (GetFlags() & FL_DISSOLVING)
		return;
...
// IdleSound 
//=========================================================
void CNPC_Terrorist::IdleSound(void)
{
	if (g_fTerroristQuestion || random->RandomInt(0, 1))
	{
		if (!g_fTerroristQuestion)
		{
...
//-----------------------------------------------------------------------------
void CNPC_Terrorist::ClearAttackConditions()
{
...
//-----------------------------------------------------------------------------
int	CNPC_Terrorist::RangeAttack2Conditions(float flDot, float flDist)
{
	return COND_NONE;
}
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_Terrorist::CanThrowGrenade(const Vector &vecTarget)
{
	if (m_iNumGrenades < 1)
	{
		// Out of grenades!
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_Terrorist::CheckCanThrowGrenade(const Vector &vecTarget)
{
	// ---------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
bool CNPC_Terrorist::CanGrenadeEnemy(bool bUseFreeKnowledge)
{
	CBaseEntity *pEnemy = GetEnemy();
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_Terrorist::MeleeAttack1Conditions(float flDot, float flDist)
{
	if (flDist > 64)
	{
...
// Output : Vector
//-----------------------------------------------------------------------------
Vector CNPC_Terrorist::EyePosition(void)
{
	if (!IsCrouching())
	{
...
// Output : Vector
//-----------------------------------------------------------------------------
Vector CNPC_Terrorist::EyeOffset(Activity nActivity)
{
	if (CapabilitiesGet() & bits_CAP_DUCK)
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
Vector CNPC_Terrorist::GetCrouchEyeOffset(void)
{
	return COMBINE_EYE_CROUCHING_POSITION;
}

//-----------------------------------------------------------------------------
void CNPC_Terrorist::OnChangeActivity(Activity eNewActivity)
{
...
//-----------------------------------------------------------------------------
void CNPC_Terrorist::SetActivity(Activity NewActivity)
{
...
//-----------------------------------------------------------------------------
NPC_STATE CNPC_Terrorist::SelectIdealState(void)
{
	switch (m_NPCState)
	{
	case NPC_STATE_COMBAT:
...
//-----------------------------------------------------------------------------
bool CNPC_Terrorist::OnBeginMoveAndShoot()
{
...
//-----------------------------------------------------------------------------
void CNPC_Terrorist::OnEndMoveAndShoot()
{
	VacateStrategySlot();
}
...
// I hate these guys...
//-----------------------------------------------------------------------------
WeaponProficiency_t CNPC_Terrorist::CalcWeaponProficiency(CBaseCombatWeapon *pWeapon)
{
	if (FClassnameIs(pWeapon, "weapon_law"))
	{
...
//-----------------------------------------------------------------------------
bool CNPC_Terrorist::ActiveWeaponIsFullyLoaded()
{
...
//-----------------------------------------------------------------------------
bool CNPC_Terrorist::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter *sourceEnt)
{
...
//-----------------------------------------------------------------------------
const char* CNPC_Terrorist::GetSquadSlotDebugName(int iSquadSlot)
{
	switch (iSquadSlot)
	{
...
//-----------------------------------------------------------------------------
bool CNPC_Terrorist::IsUsingTacticalVariant(int variant)
{
	if (variant == TACTICAL_VARIANT_PRESSURE_ENEMY && m_iTacticalVariant == TACTICAL_VARIANT_PRESSURE_ENEMY_UNTIL_CLOSE)
	{
		// Essentially fib; just say that we are using "pressure enemy".
		return true;
	}
...
//-----------------------------------------------------------------------------
bool CNPC_Terrorist::IsRunningApproachEnemySchedule()
{
...
bool CNPC_Terrorist::ShouldPickADeathPose(void)
{
	return !IsCrouching();
}
...
AI_BEGIN_CUSTOM_NPC(npc_terrorist, CNPC_Terrorist)
...
//Activities
DECLARE_ACTIVITY(ACT_TERRORIST_THROW_GRENADE)
DECLARE_ACTIVITY(ACT_TERRORIST_BLINDED)
DECLARE_ACTIVITY(ACT_TERRORIST_WALK)
...
DECLARE_CONDITION(COND_COMBINE_HIT_BY_SMOKE)
...
DECLARE_INTERACTION(g_interactionTerroristBash);
...
(
	...
	(
		...
		"		TASK_PLAY_SEQUENCE		ACTIVITY:ACT_TERRORIST_BLINDED"
		""
		"	Interrupts"
		""
		)
	...
	//
	//	Used to run straight at enemy player
	//=========================================================
	DEFINE_SCHEDULE
	(
		SCHED_COMBINE_CHARGE_PLAYER,
	...
	(
		...
		"	Tasks"
		"		TASK_SET_ACTIVITY				ACTIVITY:ACT_TERRORIST_BLINDED"
